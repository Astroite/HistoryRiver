...
> 最近归档迭代：[M1-01 核心体验原型](./iterations/archive/M1-01/README.md)
...
2. 支撑人物丝线（贯穿一生、粗细/辉光=影响力、含焦点年份节点）、策划事件聚散和思想关系等核心对象；
...
```text
src/app/
  history-river.tsx       三维场景、人物丝线着色器、焦点节点、状态控制、声音占位与指标面板
  history-geography-visuals.ts  逐年地理体积、焦点切片与山河约束渲染
  page.tsx                单路由入口与页面元数据
  globals.css             幽玄空间、信息层与桌面布局
src/lib/history/
  model.ts                模拟人物、事件、关系、领域主题色、abstractChina 地理、投影与丝线打包(buildFigureThreads)
  geography-volume.ts     历史空间关键帧、逐年插值、轮廓提取与体积几何
src/worker/
  index.ts                Cloudflare Worker 部署入口
tooling/
  sites-vite-plugin.ts    构建期打包 Sites 元数据的自定义 Vite 插件
tests/
  history-model.test.ts   数据、时间、聚集、主题色与丝线几何不变量测试
...
- `X/Z`：归一化历史地理平面，只表达相对山河方位；人物使用邦国质心，地理连续体使用区域权重场，并由黄河/长江走廊、抽象山脉与海岸方向共同约束，不用现代省界或精确坐标；
...
- `influence` 经 `activeCurveAt`（生命阶段曲线）驱动人物丝线每一段的粗细与辉光；维度切换只改着色器 `uDimWeights` 权重，不重建位置缓冲；
- 人物按领域/学派获得主题色（`themeColorFor`），同领域共享色相、个体以明度/饱和度区分；地理位置由邦国 region 质心 + 抖动派生（P0 见 `abstractChina`）；
...
| L2 焦点年份节点 | 人物在窗口中心年份的位置与身份 | 实例化点/小球，供识别与拾取 | 不可删除 |
| L3 人物丝线 | 人物贯穿一生的主题色丝线，粗细/辉光=影响力 | 批量带状几何(ribbon) + 自定义着色器（逐顶点粗细/辉光） | 不可删除，允许减少细分 |
| L4 事件场 | 聚散约束、波纹和局部水势 | 状态驱动的局部场 | 聚散不可删除，水势效果可降级 |
| L5 思想关系 | 跨人物、跨生命的细亮关系 | 独立批量线集 | 不可删除，允许只强调相关子图 |
| L6 历史地理连续体 | 每年一条抽象空间轮廓、纵向连接纤维、山河约束与焦点切片 | 单一批量 `LineSegments` + 动态焦点 `LineLoop` | 年度连续性与焦点切片不可删除，允许减少横向采样 |
| L7 后期效果 | 辉光、色差、景深等 | 最小后期链 | 优先降级 |
| L8 UI/字幕/调试 | 标签、图例、状态和指标 | DOM/CSS | 关键交互不可删除 |

### 8.2 批处理原则

- 人物丝线、背景粒子和思想线按材质/状态批处理，全部人物丝线合入单一带状几何缓冲，避免“一人一个 Object3D”；
- 每个顶点保留实体索引或可回查 ID，用于拾取和调试；
- 标签只为选中、悬停、事件核心和少量上下文对象生成；
- 窗口外对象优先降低亮度和细分，不立即从语义存储删除；
- P0 将全部人物丝线打包为单一带状几何：位置/切向/主题色/影响力基值/年份等逐顶点属性静态，粗细、辉光、窗口淡出与汇流程度由 uniform 驱动；窗口外延续以窗口淡出降低亮度而非删除几何，选中人物丝线保持全长显影；维度切换只改 `uDimWeights` uniform，不重建位置缓冲；
- 思想关系依据状态选择性显影：非关系状态只保留可发现的极弱相关子图，关系聚焦时仅高亮当前关系；
- 历史地理连续体按年生成同拓扑轮廓并合入单一 `LineSegments` 缓冲；关键帧只保存区域权重与不确定度，相邻关键帧在 CPU 上平滑插值，焦点年份轮廓使用固定动态缓冲更新；每个顶点携带 `aYear` 与 `aUncertainty`，着色器据 `年份 / 焦点年份 / 视图距离(uViewDistance) / 不确定度` 四项联合控制色温与亮度：远古偏冷蓝灰、近代偏象牙暖金，资料稀疏年份边缘更暗更冷、不伪装成确定疆界；远景强化焦点对比、贴近时抬升整体上下文以呈现沉积层体量；
- 时代过渡通过统一参数插值，避免在边界重新编译大量材质；
- 正式实现前先用内置统计确认绘制调用与顶点规模，不能只依据肉眼帧率。

### 8.3 拾取

P0 以焦点年份节点（实例化网格）为主拾取目标，对其射线拾取命中 `instanceId` 即可回查唯一 `Person`；人物丝线本体因着色器内 billboard 生成、CPU 几何为退化中心线，不直接拾取。规模实验再加入 GPU ID 缓冲或空间索引对比，但不提前锁定。

拾取验收：

- 指针目标半径与视觉节点大小分离，避免小节点不可选；
...
| 规模夹具 | 渲染与降级测试 | 最多 120,000 背景粒子、2,000 人物丝线（约 80,000 丝线顶点）、10,000 关系/山河线顶点、32 个同时可见标签 |
...
2. 降低像素比和背景雾/云采样；
3. 降低无名之流与装饰粒子密度；
4. 降低人物丝线和思想线的细分（`stepYears`/曲线采样），但保留端点、方向、证据线型与粗细/辉光语义；
5. 减少非焦点标签和远景更新频率；
6. 仅在最后降低非窗口、非选中人物的更新频率。

任何质量档都不得删除当前窗口、选中人物丝线、当前事件、当前思想关系、时间方向和返回路径。
...
- 人物丝线的带状几何采用批量 billboard ribbon + 自定义着色器（逐顶点粗细/辉光）——M1-01 结论，已实现，见 §8.1 L3；后续再评估规模夹具下的实例化段或 GPU 拾取；
...
以上条件均已满足，`M1-01-T03` 于 2026-07-16 完成。单设备性能与交互结果记录在 [M1-01 验证记录](./iterations/archive/M1-01/validation.md)；尚未完成的规模夹具、集成显卡、最终代码连续 30 秒和开发者自验复测作为归档保留项，不得视为长期性能结论。
